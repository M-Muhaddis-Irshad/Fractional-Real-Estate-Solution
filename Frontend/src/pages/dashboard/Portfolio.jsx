        ...
        <>
              ...
              <div className="tableWrap">
                <div className="tableScroll">
                  <table className="dataTable">
                    <thead>
                      <tr><th>Property</th><th>Shares</th><th>Date</th><th>Total cost</th><th>Status</th></tr>
                    </thead>
                    <tbody>
                      {pending.map((req) => (
                        <tr key={req.id}>
            ...
            <>
              ...
              <div className="tableWrap">
                <div className="tableScroll">
                  <table className="dataTable">
                    <thead>
                      <tr><th>Property</th><th>Shares owned</th><th>Invested</th></tr>
                    </thead>
                    <tbody>
                      {holdings.map((h) => (
                        <tr key={h.propertyId}>
          ...
          <div className="tableWrap">
            <div className="tableScroll">
              <table className="dataTable">
                <thead>
                  <tr><th>Property</th><th>Date</th><th>Shares</th><th>Price / share</th><th>Total</th><th>Receipt</th></tr>
                </thead>
                <tbody>
                  {transactions.map((t) => (
                    <tr key={t.id}>
                      ...
                      <td>
                        <button className="dReceiptBtn" onClick={() => setOpenTx(openTx === t.id ? null : t.id)}>
                  ...
                  {transactions.length === 0 && (
                    <tr><td colSpan={6} className="tableEmpty">No completed transactions yet.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          {transactions.map((t) =>
            openTx === t.id ? (
              <div className="card cardPad dReceipt" key={t.id}>
                ...
                <div className="cardSub">{t.date} · {t.time} · {t.shares} shares</div>
                <div className="pdReceiptRows">
                  <div><span>Price per share</span><span>{money(t.pricePerShare)}</span></div>
                  <div><span>Shares</span><span>{t.shares}</span></div>
                  <div><span>Total</span><span className="dStrong">{money(t.total)}</span></div>
                  {t.teamFee != null && <div><span>Team fee ({t.teamFeePct}%)</span><span>{money(t.teamFee)}</span></div>}
                </div>
              </div>
            ) : null
          )}
