...
export default function AdminUsers() {
  ...
  const openDetail = async (u) => {
    ...
    try {
      const res = await api(`/admin/users/${u.id}`);
      setDetailData(res);
    } catch {
      setDetailData(null);
    } finally {
    ...
    <div className="riseIn">
      ...
      <div className="toolbar">
        ...
        <div className="pillRow">
          {FILTERS.map((f) => (
            <button key={f.key} className={"pill" + (filter === f.key ? " pillActive" : "")} onClick={() => setFilter(f.key)}>
              {f.label}
            </button>
          ))}
        </div>
      ...
      <div className="tableWrap">
        <div className="tableScroll">
          <table className="dataTable">
            ...
            <tbody>
              {visible.length === 0 && (
                <tr><td colSpan={7} className="tableEmpty">No users found.</td></tr>
              )}
              {visible.map((u) => {
                  ...
                  <tr key={u.id}>
                    ...
                    <td>
                      <div className="aRowActions aRowActionsWrap">
                        <button className="btn btnGhost btnSm" onClick={() => openDetail(u)}>View</button>
                        {u.status === "pending" && <button className="btn btnSuccess btnSm" onClick={() => setUserStatus(u.id, "approve")}>Approve</button>}
                        {u.status === "pending" && <button className="btn btnDanger btnSm" onClick={() => setUserStatus(u.id, "reject")}>Reject</button>}
                        {u.status === "active" && <button className="btn btnGhost btnSm" onClick={() => setUserStatus(u.id, "suspend")}>Suspend</button>}
                        {u.status === "suspended" && <button className="btn btnSuccess btnSm" onClick={() => setUserStatus(u.id, "restore")}>Restore</button>}
                        {u.role === "user" && (
                          <button className="btn btnSoft btnSm" onClick={() => updateUser(u.id, { role: "superadmin" })}>Make admin</button>
                        )}
                        {u.role === "superadmin" && (
                          <button className="btn btnGhost btnSm" onClick={() => updateUser(u.id, { role: "user" })}>Revoke admin</button>
                        )}
                        <button className="btn btnDanger btnSm" onClick={() => deleteUser(u.id)}>Delete</button>
                      </div>
                    </td>
                  </tr>
                );
          ...
          <div className="aUserDetail">
            <div className="aUserDetailHead">
              ...
              <div>
                ...
                <div className="aUserDetailMeta">
                  <Badge status={detail.status} /> {detail.role === "superadmin" && <Badge status="active" label="Admin" />}
                </div>
              </div>
            ...
            {(txByUser.get(detail.id)?.txns || []).length === 0 ? (
              ...
              <div className="tableWrap">
                <div className="tableScroll">
                  <table className="dataTable">
                    <thead><tr><th>Property</th><th>Shares</th><th>Total</th><th>Date</th></tr></thead>
                    <tbody>
            ...
            {(reqByUser.get(detail.id) || []).length === 0 ? (
              ...
              <div className="tableWrap">
                <div className="tableScroll">
                  <table className="dataTable">
                    <thead><tr><th>Property</th><th>Shares</th><th>Total</th><th>Status</th></tr></thead>
                    <tbody>
            ...
            {detailLoading ? (
              <div className="dCardBodyEmpty"><div className="spinner spinnerSm" style={{ margin: "0 auto" }} /></div>
