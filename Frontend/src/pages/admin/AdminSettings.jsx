...
function SettingsBody({ platform, settings, users }) {
  const { savePlatform, saveSettings } = useAdmin();
  const { user, notify } = useApp();
  const [tab, setTab] = useState("general");
  const [form, setForm] = useState(() => buildForm(platform));
    ...
    <>
      ...
      {tab === "general" && (
        <form className="card cardPad aFormGrid" onSubmit={handleGeneral}>
          <label className="field"><span className="fieldLabel">Platform name</span><input className="input" value={form.platformName} onChange={set("platformName")} /></label>
          <label className="field"><span className="fieldLabel">Support email</span><input className="input" type="email" value={form.supportEmail} onChange={set("supportEmail")} /></label>
          <label className="field fieldFull"><span className="fieldLabel">Tagline</span><input className="input" value={form.tagline} onChange={set("tagline")} placeholder="Short description shown across the product" /></label>
          <label className="field"><span className="fieldLabel">Minimum investment (PKR)</span><input className="input" type="number" value={form.minInvestment} onChange={set("minInvestment")} /></label>
          <label className="field"><span className="fieldLabel">Session timeout (minutes)</span><input className="input" type="number" value={form.sessionTimeout} onChange={set("sessionTimeout")} /></label>
          ...
          <div className="aFormActions fieldFull">
            <button className="btn btnPrimary" disabled={saving}>{saving ? "Saving…" : "Save general settings"}</button>
          </div>
        </form>
      )}
      ...
      {tab === "payments" && (
        <form className="card cardPad aFormGrid" onSubmit={handlePayments}>
          <label className="field">
            ...
            <input className="input" type="number" step="0.05" min="0" max="25" value={form.teamFee ?? settings.teamFee} onChange={(e) => setForm((f) => ({ ...f, teamFee: e.target.value }))} />
          ...
          <label className="field">
            ...
            <input className="input" value={form.termsVersion ?? settings.termsVersion} onChange={(e) => setForm((f) => ({ ...f, termsVersion: e.target.value }))} />
          ...
          <div className="aFormActions fieldFull">
            <button className="btn btnPrimary" disabled={saving}>{saving ? "Saving…" : "Save payment settings"}</button>
          </div>
        </form>
      )}

      {tab === "security" && (
        <form className="card cardPad aFormGrid" onSubmit={(e) => { e.preventDefault(); notify("Security settings saved.", "success"); }}>
          <label className="field"><span className="fieldLabel">Minimum password length</span><input className="input" type="number" min="6" value={form.passwordMinLength} onChange={set("passwordMinLength")} /></label>
          <label className="aCheckField">
      ...
      {tab === "profile" && (
        <div className="grid2">
          <form className="card cardPad" onSubmit={(e) => { e.preventDefault(); notify("Profile saved.", "success"); }}>
            <div className="dFormStack">
              <div className="dProfileAvatar">
                <Avatar name={user?.name} size="lg" />
              </div>
              <label className="field"><span className="fieldLabel">Name</span><input className="input" defaultValue={user?.name} /></label>
              <label className="field"><span className="fieldLabel">Email</span><input className="input" defaultValue={user?.email} disabled /></label>
              <button className="btn btnPrimary">Save profile</button>
            </div>
          </form>
          <form className="card cardPad" onSubmit={changePassword}>
            <div className="dFormStack">
              <label className="field"><span className="fieldLabel">Current password</span><input className="input" type="password" value={pw.currentPassword} onChange={(e) => setPw((p) => ({ ...p, currentPassword: e.target.value }))} /></label>
              <label className="field"><span className="fieldLabel">New password</span><input className="input" type="password" value={pw.newPassword} onChange={(e) => setPw((p) => ({ ...p, newPassword: e.target.value }))} /></label>
        ...
        <div className="tableWrap">
          <div className="tableScroll">
            <table className="dataTable">
              <thead><tr><th>Admin</th><th>Email</th><th>Permissions</th></tr></thead>
              <tbody>
                {admins.map((a) => (
                  <tr key={a.id}>
                    <td>
                    ...
                    <td><span className="badge badgeBrand">Full access</span></td>
                  </tr>
                ))}
                {admins.length === 0 && <tr><td colSpan={3} className="tableEmpty">No admins found.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
