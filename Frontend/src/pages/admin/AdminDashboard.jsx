...
export default function AdminDashboard() {
  const { stats, requests, activity, transactions, properties, approveRequest, rejectRequest, loading } = useAdmin();
  const { user } = useApp();
    ...
    <div className="riseIn">
      <div className="pageHead">
        <div>
          ...
          <p className="pageSub">Monitor the entire Flux platform in real time, {user?.name?.split(" ")[0]}.</p>
        </div>
        <div className="aHeadActions">
          <Link to="/admin/properties/new" className="btn btnPrimary">+ List property</Link>
          <Link to="/admin/investments" className="btn btnGhost">Review requests</Link>
        </div>
      </div>
      ...
      <div className="kpiGrid">
        ...
        <Kpi label="Total invested" value={money(s.totalInvested)} sub="All-time capital deployed" accent="#6366f1" />
        <Kpi label="Total revenue" value={money(s.totalRevenue)} sub={`incl. ${moneyShort(s.teamEarnings)} fees`} accent="#10b981" />
      ...
      <div className="grid-2-1 aLowerGrid">
              ...
              <div className="aActivityRow" key={a.id}>
                ...
                <div className="aActivityBody">
                  ...
                  <div className="aActivityMeta">
                    {a.actor ? `${a.actor.name} · ` : ""}{timeAgo(a.createdAt)}
                  </div>
                </div>
        ...
        <div className="card">
          <div className="cardHead">
            ...
            <Link to="/admin/investments" className="dLink">All →</Link>
          </div>
              ...
              {pendingRequests.slice(0, 5).map((r) => (
                <div className="aApproveRow" key={r.id}>
                  <div>
                    ...
                    <div className="dMiniMeta">{r.user?.name} · {r.shares} shares · {fmtDateTime(r.createdAt)}</div>
                  </div>
      ...
      <div className="grid3">
        {[
          ...
          <Link to={q.to} className="dQuick" key={q.to}>
