        ...
        <form className="card cardPad" onSubmit={handleSend}>
          ...
          <div className="aFormGrid" style={{ marginTop: 16 }}>
            <label className="field">
              ...
              <input className="input" value={form.title} onChange={set("title")} placeholder="e.g. New property live" maxLength={120} />
            </label>
            <label className="field">
            ...
            <label className="field">
              ...
              <textarea className="textarea" value={form.message} onChange={set("message")} placeholder="Write your announcement…" maxLength={1000} />
            </label>
          </div>
          <div className="aFormActions">
            <button className="btn btnPrimary" disabled={sending || !form.title.trim() || !form.message.trim()}>
              {sending ? "Sending…" : "Send announcement"}
            </button>
          </div>
          <div className="fieldHint" style={{ marginTop: 10 }}>
            Email and push delivery require an external provider — these are recorded here for the audit trail.
          </div>
        </form>
            ...
            {notifications.map((n) => (
              <div className="dMiniRow" key={n.id}>
                <div>
                  ...
                  <div className="dMiniMeta">{fmtDateTime(n.createdAt)} · {n.audience}</div>
                  <div className="dMiniMeta" style={{ marginTop: 3 }}>{n.message}</div>
                </div>
                <div className="dMiniRight">
                  <Badge status="active" label={n.channel} />
                  <button className="btn btnDanger btnSm" onClick={() => deleteNotification(n.id)}>Delete</button>
                </div>
              </div>
            ))}
          </div>
