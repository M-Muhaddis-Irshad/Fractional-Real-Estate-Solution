...
function LogsPanel({ tab }: { tab: TabKey }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        if (tab === "errors") {
          const res = await api<{ errors?: ErrorLog[] }>("/admin/errors");
          if (active) setErrors(res.errors || []);
        } else if (tab === "login") {
          const [res, res2] = await Promise.all([
            api<{ logs?: LogEntry[] }>("/admin/logs?type=login"),
            api<{ logs?: LogEntry[] }>("/admin/logs?type=admin_login"),
          ]);
          if (active)
            setLogs(
              [...(res.logs || []), ...(res2.logs || [])].sort(
                (a, b) =>
                  new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime()
              )
            );
        } else if (tab === "activity") {
          const res = await api<{ logs?: LogEntry[] }>("/admin/logs?actor=admin");
          if (active) setLogs(res.logs || []);
        } else {
          const res = await api<{ logs?: LogEntry[] }>("/admin/logs");
          if (active) setLogs(res.logs || []);
        }
      } catch {
              ...
              {errors && errors.length === 0 && (
                <tr>
                  <td colSpan={4} className="tableEmpty">
                    No captured errors — the platform is healthy.
                  </td>
                </tr>
              )}
              {(errors || []).map((e) => (
                <tr key={e.id}>
                  <td>
                    <div className="dStrong" style={{ color: "var(--danger)" }}>
                      {e.message}
                    </div>
                  ...
                  <td>
                    <Badge status="rejected" label={e.type} />
                  </td>
          ...
          <tbody>
            ...
            {(logs || []).slice(0, 150).map((l) => (
              <tr key={l.id}>
                ...
                <td>
                  {l.actor ? <span className="badge badgeBrand">{l.actor.name}</span> : <span className="dMuted">—</span>}
                </td>
                <td>
                  <span className="badge badgeNeutral">{l.type}</span>
                </td>
