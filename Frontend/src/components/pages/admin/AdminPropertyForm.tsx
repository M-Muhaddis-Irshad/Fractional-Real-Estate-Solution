...
function PropertyFormBody({
  ...
  const set = (f: keyof PropertyFormValues) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) =>
    ...
    <form className="card cardPad aFormCard" onSubmit={handleSubmit}>
      <div className="aFormGrid">
        ...
        <label className="field" style={{ gridColumn: "1 / -1" }}>
          ...
          <input
            ...
            placeholder="https://images.unsplash.com/photo-…?auto=format&fit=crop&w=1400&q=80"
          />
          <span className="fieldHint">
            Paste a direct link to a high-resolution photo (Unsplash, Pexels, etc.). If empty, a
            branded gradient placeholder is used.
          </span>
        </label>
      </div>
      <label className="field" style={{ marginTop: 16 }}>
        ...
        <textarea
          ...
          placeholder="Describe the property, location, income potential…"
        />
      </label>

      {error && (
        <div className="errorText" style={{ marginTop: 14 }}>
          {error}
        </div>
      )}
...
export default function AdminPropertyForm({ id }: { id?: string }) {
  ...
  return (
    <div className="riseIn">
      ...
      <PropertyFormBody
        ...
        onDone={() => router.push("/admin/properties")}
      />
    </div>
  );
}
