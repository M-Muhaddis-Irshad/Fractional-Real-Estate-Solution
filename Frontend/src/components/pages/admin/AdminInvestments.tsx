    ...
    <div className="riseIn">
      ...
      <div className="kpiGrid">
        <div className="kpi">
          ...
          <div className="kpiValue">{money(totalInvested)}</div>
        </div>
        <div className="kpi">
          <div className="kpiLabel">Completed deals</div>
          <div className="kpiValue">{transactions.length}</div>
        </div>
        <div className="kpi">
          <div className="kpiLabel">Platform fees</div>
          <div className="kpiValue">{moneyShort(totalFees)}</div>
        </div>
        <div className="kpi">
                ...
                {pending.length === 0 && (
                  <tr>
                    <td colSpan={7} className="tableEmpty">
                      No pending requests — investments settle instantly.
                    </td>
                  </tr>
                )}
                {pending.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className="dMuted">
                      {money(r.teamFeeAmount || 0)} ({r.teamFeePct}%)
                    </td>
              ...
              <tbody>
                ...
                {transactions.map((t) => (
                  <tr key={t.id}>
                    ...
                    <td className="dMuted">{money(t.teamFee || 0)}</td>
                    <td className="dMuted">
                      {t.date} · {t.time}
                    </td>
                  </tr>
