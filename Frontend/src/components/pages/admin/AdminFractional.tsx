...
export default function AdminFractional() {
  ...
  const [chain, setChain] = useState<{ valid?: boolean; blockCount?: number } | null>(null);
    ...
    (async () => {
      try {
        const [tokRes, chainRes] = await Promise.all([
          ...
          api<{ valid?: boolean; blockCount?: number }>("/admin/tokens/verify"),
        ]);
        if (!active) return;
  ...
  return (
    <div className="riseIn">
      ...
      <div className="kpiGrid">
        <div className="kpi">
          <div className="kpiLabel">Total shares issued</div>
          <div className="kpiValue">{totals.totalShares}</div>
        </div>
        <div className="kpi">
          <div className="kpiLabel">Shares sold</div>
          <div className="kpiValue">{totals.soldShares}</div>
        </div>
        <div className="kpi">
          <div className="kpiLabel">Shares remaining</div>
          <div className="kpiValue">{totals.totalShares - totals.soldShares}</div>
        </div>
        <div className="kpi">
          <div className="kpiLabel">Avg. funded</div>
          <div className="kpiValue">
            {totals.totalShares ? Math.round((totals.soldShares / totals.totalShares) * 100) : 0}%
          </div>
        </div>
      </div>
            ...
            <div className={"card cardPad aFrac" + (paused ? " aFracPaused" : "")} key={p.id}>
              ...
              <div className="aFracRows">
                <div>
                  ...
                  <span className="dStrong">{money(p.pricePerShare)}</span>
                </div>
                <div>
                  <span>Total value</span>
                  <span>{moneyShort(p.totalValue)}</span>
                </div>
                <div>
                  <span>Capital raised</span>
                  <span className="dStrong">{money(invested)}</span>
                </div>
                <div>
                  <span>Yield</span>
                  <span>{p.yieldPct}% APY</span>
                </div>
              </div>
              ...
              <div className="aFracActions">
                ...
                <span className="aFracToggleLabel">
                  {paused ? "Investing paused" : "Investing open"}
                </span>
              </div>
            </div>
          );
        })}
        {properties.length === 0 && <div className="card dCardBodyEmpty">No properties listed yet.</div>}
      </div>
              ...
              {tokens.filter((t) => t.kind === "mint").length === 0 && (
                <tr>
                  <td colSpan={7} className="tableEmpty">
                    No tokens minted yet — purchases mint automatically.
                  </td>
                </tr>
              )}
            </tbody>
