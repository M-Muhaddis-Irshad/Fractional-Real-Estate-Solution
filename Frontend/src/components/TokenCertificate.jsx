...
export function TokenCard({ token, onOpen }) {
  return (
    <div
      className="tkCard"
...
export function TokenCertificate({ token, onClose }) {
  ...
  const verify = async () => {
    ...
    setResult(null);
    try {
      const data = await api("/tokens/verify", { auth: false });
      setResult(data);
    } catch {
    ...
    <Modal title={`Token certificate · ${token.tokenId}`} onClose={onClose} wide>
      <div className="tkCert">
        ...
        <div className="pdReceiptRows">
          <div><span>Owner</span><span className="dStrong">{token.ownerName}</span></div>
          <div><span>Shares</span><span>{token.shares}</span></div>
          <div><span>Price per share</span><span>{money(token.pricePerShare)}</span></div>
          <div><span>Total value</span><span className="dStrong">{money(token.totalValue)}</span></div>
          <div><span>Minted</span><span>{fmtDateTime(token.timestamp)}</span></div>
        </div>
        <div className="tkHashRows">
          <div><span>Token ID</span><span className="dMono">{token.tokenId}</span></div>
          <div><span>Transaction hash</span><span className="dMono">{token.txHash}</span></div>
          <div><span>Block</span><span className="dMono">#{token.blockNumber}</span></div>
          <div><span>Previous block hash</span><span className="dMono">{token.previousHash}</span></div>
          <div><span>Block hash</span><span className="dMono">{token.hash}</span></div>
          <div><span>Mining nonce</span><span className="dMono">{token.nonce}</span></div>
        </div>
